...
export default function InputBar() {
  ...
  const { fetchResearch, uploadFile, loading, selectedHashes } = useResearch();
  ...
  return (
    ...
    >
      ...
      {selectionActive && (
        <div className="max-w-3xl mx-auto mb-2 flex items-center gap-2 bg-blue-950/60 border border-blue-700/40 rounded-lg px-3 py-2 text-xs text-blue-300">
          <span>📌</span>
          <span>
            Querying <strong>{selectedHashes.length}</strong> selected file{selectedHashes.length > 1 ? 's' : ''} only — web search is disabled.
          </span>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto flex items-center gap-2 bg-gray-800 rounded-xl p-2 border border-gray-700 focus-within:border-blue-500 transition-all">
        ...
        <textarea
          ...
          placeholder={
            pendingFile
              ? 'Add a question about this file, or just hit send to upload…'
              : selectionActive
              ? `Ask about the ${selectedHashes.length} selected file${selectedHashes.length > 1 ? 's' : ''}…`
              : 'Ask anything…'
          }
      ...
      <p className="max-w-3xl mx-auto mt-1.5 text-xs text-gray-600 text-center">
        {selectionActive
          ? `Responses will draw only from the ${selectedHashes.length} selected document${selectedHashes.length > 1 ? 's' : ''}. Deselect in the sidebar to re-enable web search.`
          : <>Attach a <strong className="text-gray-500">PDF</strong> or <strong className="text-gray-500">.txt</strong> file — it will be indexed and searchable in your next query.</>
        }
      </p>
    </div>
  );
